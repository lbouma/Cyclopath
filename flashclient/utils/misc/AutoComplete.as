// The AutoComplete component was obtained from Adobe Flex Exchange at
// http://www.adobe.com/cfusion/exchange/index.cfm?event=extensionDetail&extid=1047291
// This is a modified version of the original component. This blog post from
// the Official Flex Team Blog talks about the component and how we can modify
// the source code:
// http://blogs.adobe.com/flex/archives/2006/09/
// The relevant quote from the post is "Finally, we will include the source
// so that you can modify the component as you see fit."
...
public class AutoComplete extends ComboBox 
{
	...
	public function AutoComplete()
	{
		...
		rowCount = 7;
		logTimer = new Timer(100, 1);
		logTimer.addEventListener(TimerEvent.TIMER, logList);

      // 2012.11.12: [lb] Do we need to call removeEventListener later?
	}
	...
	//--------------------------------------------------------------------------
        ...
        // itemName: what are the items in the collection? (e.g. tag, region)
	...
	override protected function keyDownHandler(event:KeyboardEvent):void
	{
	    ...
	    if(!event.ctrlKey)
		{
			...
			else if(event.keyCode == Keyboard.DOWN && typedText == "")
			{
			   showDropdown = true;
			}
	 	}
	...
	{
// FIXME: Is this why AutoComplete can't be 100%?
	    super.measure();
	...
	private function defaultFilterFunction(element:*, text:String):Boolean 
	{
  	...
  	//
  	private function logList(event:TimerEvent): void
  	{
 	   var s:String = "";
 	   var a:Array = (collection as ListCollectionView).toArray();
 	   
 	   // This loop is used to distinguish user-entered | from |'s used
 	   // for separating items.
 	   for (var i:String in a)
 	      a[i] = escape(a[i]);
 	   // The current row, row count, and collection length are used to figure
 	   // our which items are visible.
 	   s += a.slice(currentRow,currentRow
 	                 + Math.min(rowCount,collection.length)).join("|");
           if (itemName === null)
              throw new Error('Please define itemName');
           G.sl.event('ui/tag/autocomplete', {item: itemName, 
                                              user_text: typedText,
                                              suggested: s});
  	}
