...
sy keyword htmlTagName contained math nowiki references source
...
sy match wikiSourceTag    +<source[^>]*>+ contains=htmlTag
...
sy region wikiSource start=+<source[^>]*>+ keepend end="</source>"me=s-1 contains=wikiSourceTag
...
sy region wikiBold          start=+'''+         end=+'''+   oneline contains=@wikiTop,wikiBoldItalic,wikiNowiki,wikiNowikiEndTag
...
" a situation Vim by default doesn't handle as other text editors. Save the 
" following lines to "~/.vim/ftplugin/Wikipedia.vim" or 
" "vimfiles\ftplugin\Wikipedia.vim" to make it as you may be used to from 
" Notepad:
...
" Additional to that you may want to save the following lines to 
" "~/.vim/ftplugin/Wikipedia.vim" or "vimfiles\ftplugin\Wikipedia.vim" for some
" more gimmicks (you may need to put "filetype plugin on" in your vimrc):
